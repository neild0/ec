...
from dreamcoder.domains.regex.makeRegexTasks import makeOldTasks, makeLongTasks, makeShortTasks, makeWordTasks, \
    makeNumberTasks, makeHandPickedTasks, makeNewTasks, makeNewNumberTasks  # , add_string_constants
...
Text.ConstantInstantiateVisitor.SINGLE = \
    Text.ConstantInstantiateVisitor(list(map(list, list({tuple([c for c in s])
                                                         for t in loadPBETasks("PBE_Strings_Track")[
                                                             0] + dreamcoder.domains.text.makeTextTasks.makeTasks()
