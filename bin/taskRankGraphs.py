...
def plotTimeMetrics(
    ...
    for j, path in enumerate(resultPaths):
        ...
        for k, metricToPlot in enumerate(metricsToPlot):
            ...
            ylabel = metricToPlot
            export_name = experimentName + metricToPlot + "_iters_" + str(iterations) + "outlier_threshold_" + str(
                outlierThreshold) + "_time_plot.png"
...
def plotEmbeddingWithLabels(embeddings, labels, title, exportPath, xlabel=None, ylabel=None, colorLabeling=None):
    ...
    plot.figure(figsize=(10, 10))
    plot.tick_params(axis='both', left='off', top='off', right='off', bottom='off', labelleft='off', labeltop='off',
    ...
    for j, path in enumerate(resultPaths):
        ...
        for k, metricToCluster in enumerate(metricsToCluster):
            ...
            clusteredTaskMetrics = random_restart_tsne(taskMetrics,
                                                       ...
                                                       lr=tsneLearningRate,
                                                       restarts=10)
            title = title or ("Metric: %s, Domain: %s, Experiment: %s, Iteration: %d" % (
            metricToCluster, domain, experimentName, iterations))

            if labelWithImages or labelsAndImages:
                images = {}
                for i, task in enumerate(sorted(filter(lambda mt: isinstance(mt, Task), recognitionTaskMetrics.keys()),
                                                key=lambda
                                                        task: task.name)):  # Enumerate in same order as sorted tasks.
                    if domain == 'tower':
                ...
                if not labelsAndImages:
                    plotEmbeddingWithImages(clusteredTaskMetrics,
                                            ...
                                            taskNames,
                                            title,
                                            os.path.join(export, domain,
                                                         experimentName + metricToCluster + "_iters_" + str(
                                                             iterations) + "_tsne_images.png"))
                else:
                    plotLabeledImages(clusteredTaskMetrics,
                                      [images[n] for n in taskNames],
                                      taskNames,
                                      title,
                                      os.path.join(export, domain, experimentName + metricToCluster + "_iters_" + str(
                                          iterations) + "_tsne_labeledimages.png"),
                                      )
...
def getGroundTruthStarts(groundTruthCheckpoints):
    ...
    for j, path in enumerate(groundTruthCheckpoints):
        ...
        trainToStarts = {task: getFrontierStart(frontiers) for task, frontiers in taskSolutions.items() if
                         len(frontiers) > 0}
    ...
    for j, path in enumerate(checkpoints):
        ...
        for task in sorted(recognitionTaskMetrics.keys(), key=lambda task: formattedName('frontier', task)):
            if 'expectedProductionUses' in recognitionTaskMetrics[task]:
                ...
                testExpected.append(
                    recognitionTaskMetrics[task]['frontier'].expectedProductionUses(result.grammars[-1]))

        trainNames, trainExpected, testNames, testExpected = np.array(trainNames), np.array(trainExpected), np.array(
            testNames), np.array(testExpected)
        if ordering == 'MDS':
            ...
            trainNames, trainExpected, testNames, testExpected = trainNames[trainOrdering], trainExpected[
                trainOrdering], testNames[testOrdering], testExpected[testOrdering]

        allEPs.append([(trainNames, trainExpected, getSimilarities(trainExpected, applySoftmax)),
...
def getSimilarityMetrics(similarityMetric, recognitionTaskMetrics, trainNames, testNames, applySoftmax):
    ...
    trainMetrics = [newRecognitionMetrics[taskName][similarityMetric] for taskName in trainNames if
                    similarityMetric in newRecognitionMetrics[taskName]]
    testMetrics = []
    if similarityMetric in metricToHeldout:
        heldoutMetric = metricToHeldout[similarityMetric]
        testMetrics = [newRecognitionMetrics[taskName][heldoutMetric] for taskName in testNames if
                       heldoutMetric in newRecognitionMetrics[taskName]]
    if applySoftmax:
    ...
    testMetrics, getSimilarities(testMetrics, applySoftmax))
...
def similarityAnalysis(
    ...
    for j, path in enumerate(checkpoints):
        result, domain, iterations, recognitionTaskMetrics = loadResult(path)
        ((trainNames, trainExpected, trainTruthSims), (testNames, testExpected, testTruthSims)) = \
        ...
        for similarityMetric in similarityMetrics:
            (trainMetrics, trainMetricSims), (testMetrics, testMetricSims) = getSimilarityMetrics(similarityMetric,
            ...
            if similarityWithTSNE:
                ...
                plotEmbeddingWithLabels(clusteredTrainMetrics,
                                        trainNames,
                                        similarityMetric,
                                        os.path.join(exportPath, experimentNames[j] + "_it_%d_%s_train_tsne.png" % (
                                        iterations, similarityMetric)),
                                        colorLabeling="list")
            #       if testMetrics:
