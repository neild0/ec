...
class SyntaxCheckingRobustFill(nn.Module):
    def __init__(self, input_vocabularies, target_vocabulary, hidden_size=512, embedding_size=128, cell_type="LSTM",
        ...
        if cell_type == 'LSTM':
            self.encoder_init_h = Parameter(
                torch.rand(1, self.hidden_size))  # Also used for decoder if self.no_inputs=True
            ...
            self.syntax_decoder_cell = nn.LSTMCell(input_size=self.v_target + 1, hidden_size=self.hidden_size,
                                                   bias=True)
    ...
    def sampleAndScore(self, batch_inputs=None, n_samples=None, nRepeats=None, autograd=False, vocab_filter=None):
        ...
        inputs = self._inputsToTensors(batch_inputs)
        if nRepeats is None:
            target, score, syntax_score = self._run(inputs, mode="sample", n_samples=n_samples,
    ...
    def _fix_optstate(
            self):  # make sure that we don't have optstate on as tensor but params as cuda tensor, or vice versa
    ...
    def _run(self, inputs, target=None, mode="sample", n_samples=None, vocab_filter=None):
        ...
        if vocab_filter is not None:
            vocab_mask = self.t.new(
                [[v in V for v in self.target_vocabulary] + [True] for V in vocab_filter]).byte()  # True for STOP
        ...
        if self.no_inputs:
            ...
            inputs_scatter = [
                [Variable(self._zeros(max_length_inputs[i][j], batch_size, self.v_inputs[i] + 1).scatter_(2,
                                                                                                          inputs[i][j][
                                                                                                          :, :, None],
                                                                                                          1))
        ...
        if target is not None: target_scatter = Variable(
            self._zeros(max_length_target, batch_size, self.v_target + 1).scatter_(2, target[:, :, None],
            ...
            for j in range(n_examples):
                ...
                for k in range(max_length_inputs[i][j]):
                    if i == 0:
                        ...
                        state = self.encoder_cells[i](torch.cat([inputs_scatter[i][j][k, :, :], attend(i, j, h)], 1),
                                                      state)
                    if k + 1 < max_length_inputs[i][j]: active[k + 1, :] = active[k, :] * (
        ...
        if self.no_inputs:
            ...
            decoder_states = [self._decoder_get_init(embeddings[self.n_encoders - 1][j]) for j in
                              range(n_examples)]  # P
        ...
        for i in range(self.n_encoders):
            ...
            for j in range(len(inputsss[0])):
                ...
                for k in range(len(inputs)):
                    s = inputs[k]
                    try:
                        if len(s) > 0: t[:len(s), k] = torch.LongTensor(
    ...
    def _run_with_beam(self, inputs, beam_size=10, vocab_filter=None, maxlen=None):
        ...
        target, score, decoder_states, syntax_decoder_state, active, H, attention_mask, max_length_inputs, batch_size, n_examples = self._encode(
            inputs, vocab_filter=vocab_filter)  # use hack on run
            ...
            for target, score, decoder_states, syntax_decoder_state, active in beam:
                if not any(active == True):
                    if len(new_beam) < beam_size:
                        new_beam.append((target, score, decoder_states, syntax_decoder_state,
                                         active))  # I think it's fine not to clone here
                    ...
                    # print(k, flush=True)
                    logsoftmax = self._run_first_half(k, decoder_states, syntax_decoder_state, H, attention_mask,
                                                      max_length_inputs, batch_size, n_examples,
                                                      vocab_filter=vocab_filter)
                    for token in list(self.target_vocabulary_index.values()) + [self.v_target]:
                        # do filtering for these lines
                        candidate = self._run_second_half(k, logsoftmax, target.clone(), token, score.clone(),
                                                          [(ds[0].clone(), ds[1].clone()) for ds in decoder_states], (
                                                          syntax_decoder_state[0].clone(),
                                                          syntax_decoder_state[1].clone()), active.clone(), batch_size,
                                                          n_examples)
                        if len(new_beam) < beam_size:
    ...
    def _run_first_half(self, k, decoder_states, syntax_decoder_state, H, attention_mask, max_length_inputs, batch_size,
                        n_examples, vocab_filter=None):
        ...
        for j in range(1 if self.no_inputs else n_examples):
            ...
            p_aug = h if self.no_inputs else torch.cat(
                [h, self.attend_for_beam(self.n_encoders, j, h, H, attention_mask, max_length_inputs, batch_size)],
                1)  # will need H and attention_mask for attend fn
            FC.append(F.tanh(self.W(p_aug)[None, :, :]))
    ...
    def _run_second_half(self, k, logsoftmax, target, token, score, decoder_states, syntax_decoder_state, active,
        ...
        return target.clone(), score.clone(), [(ds[0].clone(), ds[1].clone()) for ds in decoder_states], (
        syntax_decoder_state[0].clone(), syntax_decoder_state[1].clone()), active.clone()
